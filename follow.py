...
class JacobianDemo():
    ...
    def ellipse(t,f=1,ry=.15,rz=.15):
        ...
        #Xc represents position vector from base to desired ellipse center
        xc = x0 + [0, 0, -rz]
        xdes = xc + np.array([0, ry*cos(t*f), rz*sin(t*f)])
        vdes = np.array([0,-f*ry*sin(t*f),f*rz*cos(t*f)])
    ...
    def line(t,f=1,L=.2):
        ...
        ## STUDENT CODE GOES HERE
        x0 = np.array([0.307, 0, 0.487]) # corresponds to neutral position
        xdes = x0 - np.array([0,0,-L*sin(t*f)])
        vdes = np.array([0,0,-f*L*cos(t*f)])
        ## END STUDENT CODE
    ...
    def follow_trajectory(self, state, trajectory, arm):
    ...
    def line_trajectory(self, state, arm):
        q = state['position']
        dist = 0.2

        for i in np.arange(0,dist, 0.001):
            v = np.array([0, 0, - (0.1 - i/dist)])
            dq = IK_velocity(q,v,np.array([np.nan,np.nan,np.nan]))
            new_q = q + dq
            arm.safe_set_joint_positions_velocities(new_q, dq)
        
        #arm.safe_set_joint_positions_velocities(q, np.zeros((1, 7)))
